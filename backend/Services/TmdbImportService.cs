...
public class TmdbImportService : ITmdbImportService
{
    ...
    // Normalise a few TMDB genre names to shorter, friendlier local names.
    ...
    // ── TMDB response DTOs (private, only used here) ──────────────────────────

    private record TmdbGenre(int Id, string Name);
    ...
    private record TmdbShowDetails(
        int NumberOfSeasons,
