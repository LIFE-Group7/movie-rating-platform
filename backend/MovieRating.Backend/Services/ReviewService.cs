...
public class ReviewService : IReviewService
{
    ...
    public async Task<Result<IEnumerable<MovieReviewDto>>> GetMovieReviewsAsync(int movieId)
    {
        try
        {
            var reviews = await _repository.GetMovieReviewsAsync(movieId);

            var reviewItems = reviews.Select(review => new MovieReviewDto
            {
                MovieId = review.MovieId,
            ...
            return Result<IEnumerable<MovieReviewDto>>.Success(reviewItems);
        }
        catch (Exception exception)
        {
            _logger.LogError(exception, "Failed to retrieve reviews for Movie {MovieId}", movieId);
            return Result<IEnumerable<MovieReviewDto>>.Failure("An unexpected error occurred.", ErrorType.Failure);
        }
    }
    ...
    public async Task<Result<IEnumerable<UserShowResponseReviewDto>>> GetShowUserReviewsAsync(int userId)
    {
        try
        {
            ...
            var reviewItems = reviews.Select(review => new UserShowResponseReviewDto()
            {
                ...
                ShowCoverImageUrl = review.Show.CoverImageUrl,
                Rating = review.Rating,
                Comment = review.Comment,
                CreatedAt = review.CreatedAt,
                UpdatedAt = review.UpdatedAt
    ...
    public async Task<Result<IEnumerable<ShowReviewDto>>> GetShowReviewsAsync(int showId)
    {
        try
        {
            var reviews = await _repository.GetShowReviewsAsync(showId);

            var reviewItems = reviews.Select(review => new ShowReviewDto
            {
                ShowId = review.ShowId,
                Author = new ReviewAuthorDto
                {
            ...
            return Result<IEnumerable<ShowReviewDto>>.Success(reviewItems);
        }
        catch (Exception exception)
        {
            _logger.LogError(exception, "Failed to retrieve reviews for Show {ShowId}", showId);
            return Result<IEnumerable<ShowReviewDto>>.Failure("An unexpected error occurred.", ErrorType.Failure);
        }
    }
